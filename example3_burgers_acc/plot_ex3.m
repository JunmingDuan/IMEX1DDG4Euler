function plot_ex2(K, n, limiter);
% para: n, P_n polynomial;

addpath('../src/');
format long ;
f = @(x) sqrt(8.*cos(x/4).*(cos(x/4).*cos(x/4)-3)/3+16./3);
err = zeros(5,3);

switch n;
case 20;
  numer1 = load(['example3_Nx20_K',num2str(K),'_PP',num2str(limiter),'.dat']);
  ...
  disp(err);
case 40;
  numer2 = load(['example3_Nx40_K',num2str(K),'_PP',num2str(limiter),'.dat']);
  ...
  disp(err);
case 80;
  numer3 = load(['example3_Nx80_K',num2str(K),'_PP',num2str(limiter),'.dat']);
  ...
  disp(err);
case 160;
  numer4 = load(['example3_Nx160_K',num2str(K),'.dat']);
...
case 0;
  ...
  numer2 = load(['example3_Nx40_K',num2str(K), '_PP',num2str(limiter),'.dat']);
  numer3 = load(['example3_Nx80_K',num2str(K), '_PP',num2str(limiter),'.dat']);
  %numer4 = load(['example3_Nx160_K',num2str(K),'_PP',num2str(limiter),'.dat']);
  %numer5 = load(['example3_Nx320_K',num2str(K),'.dat']);
  x1 = numer1(:,1); y1 = numer1(:,3);
  x2 = numer2(:,1); y2 = numer2(:,3);
  x3 = numer3(:,1); y3 = numer3(:,3);
