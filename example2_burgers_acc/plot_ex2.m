...
case 320;
  ...
  disp(err);
case 0;
  numer1 = load(['example2_Nx20_K',num2str(K), '_PP',num2str(limiter),'.dat']);
  numer2 = load(['example2_Nx40_K',num2str(K), '_PP',num2str(limiter),'.dat']);
  numer3 = load(['example2_Nx80_K',num2str(K), '_PP',num2str(limiter),'.dat']);
  numer4 = load(['example2_Nx160_K',num2str(K),'_PP',num2str(limiter),'.dat']);
  %numer5 = load(['example2_Nx320_K',num2str(K),'.dat']);
  x1 = numer1(:,1); y1 = numer1(:,3);
  x2 = numer2(:,1); y2 = numer2(:,3);
  x3 = numer3(:,1); y3 = numer3(:,3);
  ...
  ex4 = f(x4);
  %ex5 = f(x5);
  plot(x4, f(x4), '-k', x1, y1, 'o', x2, y2, '*', x3, y3, '--', x4, ...
  y4, '^');%, x5, y5, 'v');
  %cal_order
  nx = 20; h = 1/nx;
  err(1,1) = cal_norm(ex1-y1, numer1(:,2), h, 2);
